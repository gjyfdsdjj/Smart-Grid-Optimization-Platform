...
## 지도 구현 권장 구조
- 지도 구현 기본 전략은 `대한민국 전체 원본 3D 지형 적재`가 아니라 `다중 해상도(LOD) + 영역별 정밀 조회`다.
- 대한민국 전체 지형은 거시 관찰용 `coarse terrain mesh` 또는 폴리곤화된 요약 지형으로 먼저 다루고, 화면 표현에 필요한 수준까지만 로드한다.
- 현재 화면의 `viewport` 또는 사용자가 선택한 관심 영역만 중간 해상도 타일과 벡터 레이어를 추가 로드한다.
- 사용자가 설치 지점을 클릭하거나 후보지를 확정하는 순간에만 해당 위치 주변의 고해상도 지형 또는 고도 데이터를 조회해 최종 `x, y, z`를 확정한다.
- 지도 렌더링용 형상과 분석용 좌표는 분리한다. 화면에는 단순화된 폴리곤이나 mesh를 써도 되지만, 계산에는 정밀 좌표를 사용한다.
- 전국 단위 원본 데이터를 한 번에 메모리에 올리는 접근은 기본안으로 삼지 않는다. 브이월드 호출은 가능하면 `tile`, `bbox`, `관심영역` 단위로 잘라서 사용한다.

## 지도 구현 중요 포인트
- 폴리곤화 또는 mesh 단순화는 `표현 최적화` 수단이지 `최종 입지평가` 기준이 아니다.
- 경사, 능선, 절개지, 고도 차이처럼 송전탑 설치 판단에 직접 영향을 주는 요소는 클릭 지점 주변의 정밀 지형 데이터로 다시 확인해야 한다.
- `x, y, z`는 최종적으로 설치 또는 분석에 사용할 좌표이며, 화면에 보이는 단순화 geometry의 중심점이나 꼭짓점으로 대체하지 않는다.
- 후속 스키마를 설계할 때는 좌표값 외에 `고도 산정 방식`, `좌표계`, `조회 시점`, `원본 해상도 또는 fallback 여부`를 함께 남길 수 있게 준비한다.
- Streamlit rerun 환경에서는 지도 상태, 선택 지점, 마지막 유효 지형 조회 결과를 `st.session_state` 또는 캐시로 유지해 불필요한 재조회가 반복되지 않게 한다.
- 3D 표출 또는 고도 조회가 불안정하면 전체 기능을 멈추지 말고 `map_2_5d` fallback으로 낮춘 뒤, 어떤 정보를 단순화했는지 `warnings`와 `fallback.reason`에 남긴다.
- 발전소, 송전탑, 시작점, 종료점 같은 핵심 객체는 전역 개요에서도 보이도록 유지하되, 세부 지형은 확대와 선택 단계에서만 점진적으로 정밀화한다.
