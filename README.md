...
실제 송전망 원장 데이터, 선로별 실측 조류, 운영 제약 조건이 확보되면 현재의 GridNode/GridLine/Scenario 계약과 데이터 어댑터 구조를 유지한 채 실제 데이터 기반 브리핑 도구로 확장할 수 있다. 다만 이 경우에도 실제 운영 시스템을 대체하기보다는 정책 브리핑, 시나리오 비교, 투자 검토 초기 단계의 의사결정 지원 도구로 보는 것이 적절하다.
